...
class VideoSplitFrameDataset(Dataset):
    ...
    Modes:
    - rgb_only=True: Only RGB (3 channels) - baseline ablation
    - rgb_only=False, use_flow=False: RGB + FFT + Residual (5 channels) - main model
    - rgb_only=False, use_flow=True: RGB + FFT + Residual + Flow (7 channels) - temporal model
    """

    def __init__(self, split="train", features_root="data/features", use_flow=False, rgb_only=False, transform=None):
        self.samples = []
        ...
        for video_dir in self.video_dirs:
            ...
            for frame_path in sorted(frame_root.glob("frame_*.jpg")):
                ...
                # For RGB-only mode, only require the frame to exist
                if self.rgb_only:
                    self.samples.append((frame_path, fft_path, res_path, flow_path, label))
                elif fft_path.exists() and res_path.exists() and (not self.use_flow or flow_path.exists()):
                    self.samples.append((frame_path, fft_path, res_path, flow_path, label))
    ...
    def __getitem__(self, idx):
        ...
        # RGB-only mode: just return 3-channel RGB
        if self.rgb_only:
            x = rgb
        else:
            # FFT + residual
